<div class="overflow-x-auto bg-white dark:bg-gray-800 rounded-xl">
   <!-- custom toast -->
   <x-toast/>
      <div class="w-full flex justify-between items-center mb-8 gap-8 mt-4 pr-4 pl-3">
         <div>
            <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Classification</h3>
            <p class="text-slate-500 dark:text-slate-400">Overview of all the classifications.</p>
         </div>
         <div class="ml-3">
            <div class="w-full max-w-sm min-w-[200px] relative flex">
               <div class="relative">
                  ...
                  <input
                     class="peer h-full w-full rounded-[7px] border border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 !pr-9 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50 dark:border-gray-700 dark:text-gray-200 dark:placeholder-shown:border-gray-700 dark:focus:border-gray-500"
                     placeholder=" " />
                     <label
         ...
         <table class="w-full text-left table-auto min-w-max">
            <thead>
               <tr>
                     ...
                     <th class="p-4 border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                        <p class="text-sm font-normal leading-none text-slate-500 dark:text-slate-400">Sub-classification</p>
                     </th>
            ...
            <tbody id="officeTableBody">
               @foreach ($categories as $category)
                     <tr class="hover:bg-slate-50 dark:hover:bg-slate-700 border-b border-slate-200 dark:border-slate-700">
                        <td class="p-4 py-5">
                           <p class="block font-semibold text-sm text-slate-800 dark:text-white">{{ $category->id }}</p>
                        </td>
                        <td class="p-4 py-5">
                           <p class="text-sm text-slate-500 dark:text-slate-400">{{ $category->name }}</p>
                        </td>
                        <td class="p-4 py-5">
                           <p class="text-sm text-slate-500 dark:text-slate-400">{{ $category->sub_class }}</p>
                        </td>
                        <td class="p-4 py-5 text-center flex justify-center">
                           <button data-tooltip-target="edit-tooltip-{{ $category->id }}"
                                 class="editClassButton text-center relative rounded-full p-2 transition-all duration-300 hover:bg-gray-200 active:bg-gray-300 dark:hover:bg-gray-700 dark:active:bg-gray-600"
                                 data-id="{{ $category->id }}"
                           ...
                           <!-- Edit Tooltip -->
                           <div id="edit-tooltip-{{ $category->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-800">
                              Edit
                              <div class="tooltip-arrow" data-popper-arrow></div>
                           </div>

                           <button data-tooltip-target="delete-tooltip-{{ $category->id }}"
                                 class="delete-category relative rounded-full p-2 transition-all duration-300 hover:bg-gray-200 active:bg-gray-300 dark:hover:bg-gray-700 dark:active:bg-gray-600"
                           ...
                           <!-- Delete Tooltip -->
                           <div id="delete-tooltip-{{ $category->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-800">
                              Delete
                              <div class="tooltip-arrow" data-popper-arrow></div>
                           </div>
                     </td>
            ...
            <div class="flex flex-col items-center space-y-4 p-4 ">
               <nav aria-label="Page navigation example">
                  <ul class="inline-flex -space-x-px text-sm">
                      ...
                      @else
                          <li>
                              <a href="{{ $categories->previousPageUrl() }}" class="px-3 h-8 flex items-center justify-center text-gray-500 bg-white border border-gray-300 rounded-s-lg
                      ...
                      @foreach(range(1, $categories->lastPage()) as $i)
                          <li>
                              <a href="{{ $categories->url($i) }}" class="px-3 h-8 flex items-center justify-center border border-gray-300
                                  {{ $i == $categories->currentPage() ? 'text-blue-600 bg-blue-50 dark:bg-gray-700 dark:text-white' : 'text-gray-500 bg-white hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                  {{ $i }}
                              </a>
                          </li>
                      @endforeach
                      ...
                      @if ($categories->hasMorePages())
                          <li>
                              <a href="{{ $categories->nextPageUrl() }}" class="px-3 h-8 flex items-center justify-center text-gray-500 bg-white border border-gray-300 rounded-e-lg
                              hover:bg-gray-100 hover:text-gray-700
